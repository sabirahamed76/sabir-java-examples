...
import com.home.sabir.batch.admin.model.ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl;

public class ADMIN_D_P_SEP_Caye_Rate_Info_Upd_Admin_Txn_Extractor_Service implements ResultSetExtractor<Long> {

	private static final Logger logger = LogManager.getLogger(ADMIN_D_P_SEP_Caye_Rate_Info_Upd_Admin_Txn_Extractor_Service.class);

	private ADMIN_D_P_SEP_Caye_Rate_Info_Service service;
	private DataSource dataSource;
	private String transDate;

	public ADMIN_D_P_SEP_Caye_Rate_Info_Upd_Admin_Txn_Extractor_Service(ADMIN_D_P_SEP_Caye_Rate_Info_Service service, DataSource dataSource, String transDate) {
	...
	public Long extractData(ResultSet rs) throws SQLException, DataAccessException {
		Long count = 0L;
		Set<ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl> sepCayeRateInfoInsMdlSet = new HashSet<>();
		Set<ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl> sepCayeRateInfoUpdMdlSet = new HashSet<>();
		List<SqlParameterSource> sepCayeRateInfoUpdList = new ArrayList<>();
		List<SqlParameterSource> sepCayeRateInfoInsList = new ArrayList<>();

		ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl sepCayeRateInfoInsMdl = new ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl();
		ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl sepCayeRateInfoUpdMdl = new ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl();
		...
		while (rs.next()) {
			sepCayeRateInfoUpdMdl = new ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl();
			String nric = "";
			...
			logger.info("SEP NRIC : " + rs.getString("NRIC") + " :: CAYE_CON_RATE : " + rs.getDouble("CAYE_CON_RATE")
					+ " :: sepCayeRateInfoId :" + sepCayeRateInfoId);

			if(CollectionUtils.isNotEmpty(sepCayeRateInfoIList))
				sepCayeRateInfoId = sepCayeRateInfoIList.get(0);
				
			if (sepCayeRateInfoId == null) {
				sepCayeRateInfoInsMdl = new ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl();
			...
			if (!sepCayeRateInfoInsMdlSet.isEmpty()) {
				...
				for (ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl model : sepCayeRateInfoInsMdlSet.stream().collect(Collectors.toList()))
				...
				for (ADMIN_D_P_SEP_CAYE_RATE_INFO_Mdl model : sepCayeRateInfoUpdMdlSet.stream().collect(Collectors.toList()))
		...
		service.updateAdminCayeTxn(this.transDate);
		return count;
	}
}
